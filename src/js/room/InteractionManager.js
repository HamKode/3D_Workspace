...
export class InteractionManager {
  ...
  register(meshOrGroup, id, label) {
    ...
    if (meshOrGroup.isMesh) {
      ...
      meshOrGroup.traverse((c) => { if (c.isMesh) meshes.push(c) })
    }
    meshes.forEach((m) => {
      m.userData.interactableId    = id
      m.userData.interactableLabel = label
  ...
  _toggleLamp() {
    ...
    if (this.lampGroup?.userData.bulbMat) {
      const bm = this.lampGroup.userData.bulbMat
      bm.emissiveIntensity = this.lampOn ? 1 : 0
      bm.color.set(this.lampOn ? 0xfffde7 : 0x333333)
    }

    // Shade tint
    if (this.lampGroup?.userData.shadeMat) {
      this.lampGroup.userData.shadeMat.color.set(
        this.lampOn ? 0xf5e6c8 : 0x2a2a2a
      )
    }
  ...
  _focusCamera(target) {
    ...
    if (!t) return
    this.onCameraFocus?.(t.pos, t.look)
    this._showToast(
      target === 'chair'
        ? '🪑 Camera focused on desk area — press any move key to return'
        : '🖥 Desk view — press any move key to return'
    )
  }
  ...
  _drawArt() {
    ...
    if (this._artMode === 'flow') {
      ...
      // Sacred geometry — nested polygons + concentric rings
      const cx = W / 2, cy = H / 2
      ...
      ;[3, 4, 5, 6, 7, 8].forEach((s, i) => {
        drawPoly(cx, cy, 20 + i * 18, s, rng(seed + i) * Math.PI, palette[i % palette.length], 0.7)
      })
      for (let i = 0; i < 12; i++) {
      ...
      // Layered sine waves with interference
      const layers = 12
